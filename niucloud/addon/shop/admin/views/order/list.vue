					...
					<div v-if="!orderTable.loading">
						<template v-if="orderTable.data.length">
							<div v-for="(item, index) in orderTable.data" :key="index">
								...
								<div v-if="item.shop_remark" class="text-[14px] h-[30px] leading-[30px] px-3 bg-[#fff0e5] text-[#ff7f5b]"><span class="mr-[5px]">{{ t('notes') }}：</span><span>{{ item.shop_remark }}</span></div>
							</div>
						</template>
...
import { t } from '@/lang'
import { getOrderList, getOrderStatus, getOrderType, orderClose, orderFinish, getOrderPayType, getOrderFrom } from '@/shop/api/order'
...
const loadOrderList = (page: number = 1) => {
    ...
    getOrderList({
        ...
        orderTable.data = res.data.data.map(el => {
            el.order_goods.forEach(v => {
