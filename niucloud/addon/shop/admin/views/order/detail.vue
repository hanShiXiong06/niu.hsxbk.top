    ...
    <div class="main-container">
        ...
        <el-form :model="formData" label-width="100px" ref="formRef" class="page-form" v-loading="loading"
            ...
            <el-card class="box-card !border-none relative" shadow="never" v-if="formData">
                ...
                <el-row class="row-bg px-[30px] mb-[20px]" :gutter="20">
                    <el-col :span="8">
						...
						<!-- <el-form-item :label="t('orderType')">
						    <div class="input-width">{{ formData.order_type_name }}</div>
						</el-form-item> -->
				...
				<div class="mb-[20px]">
					<p><span class="ml-[30px] text-[14px] mr-[20px]">{{ t('orderStatus') }}：</span><span class="text-[14px]">{{ formData.status_name.name }}</span></p>
					<div class="flex mt-[10px]">
						...
						<div class="flex" v-if="formData.order_delivery">
							<template v-for="(item, index) in formData.order_delivery" :key="index">
								<span v-if="item.delivery_type == 'express'" class="text-[14px] px-[15px] py-[5px] ml-[30px] text-[#ff7f5b] bg-[#fff0e5] cursor-pointer" @click="packageEvent(item.id, formData.taker_mobile)">{{ t('package') }}{{ index+1 }} </span>
							</template>
						</div>
					</div>
...
import { ref } from 'vue'
import { t } from '@/lang'
import { getOrderDetail, orderClose, orderFinish } from '@/shop/api/order'
import DeliveryAction from '@/shop/views/order/components/delivery-action.vue'
import OrderNotes from '@/shop/views/order/components/order-notes.vue'
import deliveryPackage from '@/shop/views/order/components/delivery-package.vue'
