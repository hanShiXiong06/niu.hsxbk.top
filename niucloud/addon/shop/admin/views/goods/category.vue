                ...
                <el-table :data="categoryTable.data" ref="tableRef" size="large" v-loading="categoryTable.loading"
                    ...
                    <el-table-column :label="t('categoryName')" min-width="120">
                        <template #default="{ row }">
                            <i class="order-0 iconfont icontuodong vues-rank cursor-move mr-[8px]"></i>
                    ...
                    <el-table-column prop="is_show" :label="t('isShow')" width="400">
                        <template #default="{ row }">
                            <el-tag class="cursor-pointer" :type="row.is_show != 2 ? 'success' : 'danger'"
                                @click="showClick(row)">{{ row.is_show != 2 ? '是' : '否'
                                }}</el-tag>
                        </template>
                    </el-table-column>
...
import { t } from '@/lang'
import { getCategoryTree, deleteCategory, updateCategory, editCategory } from '@/shop/api/goods'
import { img } from '@/utils/common'
import { ElMessageBox } from 'element-plus'
import categoryEdit from '@/shop/views/goods/components/category-edit.vue'
...
/**
  ...
  * @return <Array> 返回数据
  */
const treeToTile = (treeData, childKey = 'child_list') => {
    const arr = []
    const expanded = data => {
