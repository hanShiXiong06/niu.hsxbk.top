           ...
           <el-card class="box-card !border-none my-[10px] table-search-wrap" shadow="never">
                <el-form :inline="true" :model="evaluateTable.searchParam" ref="searchFormRef">
					<el-form-item :label="t('goodsName')" prop="goods_name">
					    <el-input v-model="evaluateTable.searchParam.goods_name" clearable :placeholder="t('goodsNamePlaceholder')" class="input-width" />
										...
										<template #error>
											<div class="image-slot">
												<img class="w-[50px] h-[50px]"
													 src="@/shop/assets/goods_default.png"/>
											</div>
										</template>
									</el-image>

									<img v-else class="w-[50px] h-[50px]" src="@/shop/assets/goods_default.png"
									     fit="contain"/>
					    		</div>
					    		<div class="flex">
					    	...
					    	<div>
					    		...
					    		<div class="flex flex-wrap mt-[10px]" v-if="row.image_small?.length > 0">
					    			<div v-for="(imageItem, imageIndex) in row.image_small" :key="imageIndex" class="mr-4">
										<el-image v-if="imageItem" class="w-[40px] h-[40px]"
												  :src="img(imageItem)"
												  fit="contain">
											<template #error>
												<div class="image-slot">
					    				...
					    				<img v-else class="w-[40px] h-[40px]" src="@/shop/assets/goods_default.png"/>
					    			</div>
					    		</div>
		...
		<el-dialog v-model="replyShowDialog" :title="t('explainFirst')" width="460px" class="diy-dialog-wrap" :destroy-on-close="true">
		    <el-form :model="formData" label-width="90px" ref="formRef" :rules="formRules" class="page-form">
				<el-form-item :label="t('explainFirst')" prop="explain_first">
					<el-input v-model="formData.explain_first" type="textarea" rows="4" clearable :placeholder="t('explainFirstPlaceholder')" class="input-width"/>
				</el-form-item>
		    </el-form>
		    <template #footer>
...
import { t } from '@/lang'
import { getEvaluateList, deleteEvaluate, adoptEvaluate, refuseEvaluate, replyEvaluate, toppingEvaluate, cancelToppingEvaluate } from '@/shop/api/goods'
import EvaluateAdd from '@/shop/views/goods/components/evaluate-add.vue'
...
// 置顶、取消置顶
const toppingEvent = (id: number, event) => {
    if (event == 'topping') {
