        ...
        <el-card class="box-card !border-none" shadow="never">
            <el-form :model="formData" label-width="140px" ref="formRef" :rules="formRules" class="page-form">
                ...
                <el-form-item :label="t('storeMobile')" prop="store_mobile">
                    <el-input v-model="formData.store_mobile" clearable :placeholder="t('storeMobilePlaceholder')" class="input-width" onkeyup="this.value = this.value.replace(/[^\d\.]/g,'');" />
                </el-form-item>
				...
				<el-form-item :label="t('storeAddress')" prop="address_area">
					<el-select v-model="formData.province_name" value-key="id" clearable class="w-[200px]" @change="checkCity">
					    ...
					    />
					    <el-option
							v-for="(provinceId, provinceIndex) in areaList.province " :key="provinceIndex"
					...
					<el-select v-model="formData.city_name" value-key="id" clearable class="w-[200px] ml-3" @change="checkDistrict">
					    ...
					    />
					    <el-option
							v-for="(cityId, cityIndex) in areaList.city " :key="cityIndex"
					        :label="cityId.name"
					        :value="cityId"
					    />
					</el-select>
					<el-select v-model="formData.district_name" value-key="id" clearable class="w-[200px] ml-3" @change="check">
					    ...
					    />
					    <el-option
							v-for="(districtId, districtIndex) in areaList.district " :key="districtIndex"
...
import { getStoreInfo, addStore, editStore } from '@/shop/api/delivery'
...
const checkAddressInfo = (lat, lng, type) => {
    getContraryAddress({
        location: lat + ',' + lng
    }).then(res => {
