		...
		<view class="pb-[200rpx]" v-if="type != 'logistics'">
			...
			<view class="bg-[#fff] mx-[30rpx] p-[30rpx] mt-[30rpx] rounded-[10rpx]">
				<view
					...
					<view>{{t('record')}}</view>
					<view class="flex items-center" @click="redirect({url: '/shop/pages/refund/log', param: { order_refund_no: orderRefundNo }})">
						<text>{{t('check')}}</text>
			...
			<view class="flex tab-bar justify-between items-center bg-[#fff] fixed left-0 right-0 bottom-0 min-h-[100rpx] px-1 flex-wrap">
				<view class="flex ml-[30rpx] w-[70rpx] flex-col justify-center items-center" @click="redirect({ url: '/shop/pages/index' });">
...
import { getRefundDetail, refundDelivery, editRefundDelivery, closeRefund } from '@/shop/api/refund';
...
const deliverySave = ()=>{
	deliveryForm.value.validate().then(res => {
		...
		api.then((res) => {
			setTimeout(()=>{
				redirect({ url: '/shop/pages/refund/list' })
			},500)
		}).catch(() => {
		})
	})
