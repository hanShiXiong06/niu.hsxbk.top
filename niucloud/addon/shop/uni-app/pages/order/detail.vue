		...
		<view v-if="!loading" class="pb-20rpx">
				...
				<view class="order-goods-item flex  justify-between flex-wrap mt-[50rpx]"
					...
					<view class="flex justify-end  self-end w-[100%] mt-[10rpx]">
						<view
							...
							@click="redirect({ url: '/shop/pages/refund/detail', param: { order_refund_no : goodsItem.order_refund_no } })">查看退款</view>
						<view
			...
			<view
				...
				<view class="flex justify-end mr-[30rpx]">
					...
					<view
						class="inline-block text-[26rpx] leading-[56rpx] px-[30rpx] border-[3rpx] border-solid border-[#999] rounded-full"
						v-if="detail.status == 5" @click="orderBtnFn('evaluate')">{{ t('evaluate') }}</view>
				</view>
			</view>
		</view>
...
import { getShopOrderDetail, orderClose, orderFinish } from '@/shop/api/order';
import { useSubscribeMessage } from '@/hooks/useSubscribeMessage';
import logisticsTracking from '@/shop/pages/order/components/logistics-tracking/logistics-tracking.vue'
...
const orderBtnFn = (type = '') => {
	if (type == 'pay')
		payRef.value?.open(detail.value.order_type, detail.value.order_id);
	else if (type == 'close') {
		close(detail.value);
	} else if (type == 'finish') {
		finish(detail.value);
		...
		if (!detail.value.is_evaluate) {
			redirect({ url: '/shop/pages/evaluate/order_evaluate', param: { order_id: detail.value.order_id } })
		} else {
			redirect({ url: '/shop/pages/evaluate/order_evaluate_view', param: { order_id: detail.value.order_id } })
		}
	}
}
...
<style>@import '@/shop/styles/common.scss';</style>
