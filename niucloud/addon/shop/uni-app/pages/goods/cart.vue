<template>
	<view class="bg-page h-screen overflow-hidden flex flex-col" v-if="!loading">
		<view v-if="!info" class="flex-1 flex flex-col justify-center">
			<u-empty mode="data" text="未登录">
				<u-button text="去登录" type="primary" shape="circle" size="normal" class="!w-[200rpx] mt-[20rpx]"
					@click="toLogin"></u-button>
			</u-empty>
		</view>
		<view v-else-if="!cartList.length" class="flex-1 flex flex-col justify-center">
			<u-empty mode="car"></u-empty>
			<u-button text="去逛逛" type="primary" shape="circle" size="normal" class="!w-[200rpx] mt-[20rpx]"
				@click="redirect({ url: '/shop/pages/goods/list' })"></u-button>
		</view>
		<block v-else>
					...
					<view class="py-[20rpx] px-[24rpx]">
						<uni-swipe-action>
							<view v-for="(item, index) in cartList"
								:class="['bg-[#fff] py-[30rpx] rounded-[18rpx] overflow-hidden', index ? 'mt-[20rpx]' : '']">
								<uni-swipe-action-item>
									<view :class="['flex px-[24rpx]']">
										...
										<u--image width="168rpx" height="168rpx"
											:src="img(item.goodsSku.sku_image_thumb_mid)" model="aspectFill">
											<template #error>
										...
										<view class="flex flex-1 flex-wrap ml-[20rpx]">
											<view class="w-[100%]">
												<view
													class="w-[404rpx] multi-hidden text-[#333] text-[26rpx] leading-[36rpx]">
											...
											<view class="flex justify-between items-end self-end w-[100%]">
												<view class=" text-[var(--price-text-color)] leading-[40rpx] price-font">
													...
													<text class="text-[32rpx] mr-[10rpx] font-500">{{
														item.goodsSku.sale_price }}</text>
												</view>
									...
									<template #right>
										<view
											class="w-[80rpx] h-[100%] flex items-center justify-center bg-[var(--primary-color)] text-[#fff] text-[28rpx]"
											@click="swipeClick(index, 'cart')"><text>删除</text></view>
									</template>
								</uni-swipe-action-item>
							</view>
							<view v-for="(item, index) in invalidList"
								:class="['bg-[#fff] py-[30rpx] rounded-[18rpx] overflow-hidden mt-[20rpx]']">
								<uni-swipe-action-item>
									<view :class="['flex px-[24rpx]']">
										<text
											class="self-center iconfont text-[37rpx] mr-[24rpx] iconcheckbox_nol bg-[#f5f5f5] text-[#eee] rounded-[50%]">
										</text>
									...
									<template #right>
										<view
											class="w-[80rpx] h-[100%] flex items-center justify-center bg-[var(--primary-color)] text-[#fff] text-[28rpx]"
											@click="swipeClick(index, 'invalid')"><text>删除</text></view>
									</template>
								</uni-swipe-action-item>
							</view>
						</uni-swipe-action>
	...
	<view v-if="cartList.length"
		...
		<view class="flex items-center px-[30rpx]" @click="selectAll">
			<text class="iconfont text-color text-[34rpx] mr-[12rpx]"
				:class="cartList.length == checkedNum ? 'iconxuanze1' : 'iconcheckbox_nol'"></text>
	...
	<!--  #ifndef  H5 -->
	<view v-if="cartList.length"
		class="flex h-[100rpx] items-center bg-[#fff] fixed left-0 right-0 bottom-[100rpx] box-solid mb-[env(safe-area-inset-bottom)]">
		<view class="flex items-center px-[30rpx]" @click="selectAll">
			<text class="iconfont text-color text-[34rpx] mr-[12rpx]"
				:class="cartList.length == checkedNum ? 'iconxuanze1' : 'iconcheckbox_nol'"></text>
			<text class="font-500 text-[#676767] text-[25rpx]">全选</text>
	...
	<!--  #endif -->
	<u-loading-page :loading="optionLoading" loading-text="" bg-color="none" loadingColor="var(--primary-color)"
		iconSize="35"></u-loading-page>
	<u-loading-page :loading="loading" loading-text="" bg-color="#f7f7f7" loadingColor="var(--primary-color)"
		iconSize="35"></u-loading-page>
	<tabbar />
</template>
...
import { getCartGoodsList } from '@/shop/api/cart'
...
const toLogin = () => {
	useLogin().setLoginBack({ url: '/shop/pages/goods/cart' })
}
...
const swipeClick = (index: number, type: string) => {
...
const settlement = () => {
	...
	uni.setStorage({
		...
		success() {
			redirect({ url: '/shop/pages/order/payment' })
		}
	})
}
