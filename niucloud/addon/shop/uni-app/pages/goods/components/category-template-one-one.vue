		...
		<view class="mescroll-box bg-[#F4F6F8]"
			...
			<mescroll-body ref="mescrollRef" :down="{ use: false }" @init="mescrollInit" @up="getListFn">
				<view v-if="config.search.control" class="search-box z-2 bg-[#fff] fixed top-0 left-0 right-0">
					...
					<view class="flex items-center h-[70rpx] absolute right-[48rpx] top-[18rpx]">
				...
				<view class="tabs-box z-2 fixed left-0 bg-[#fff] bottom-[50px] top-0"
					...
					<scroll-view :scroll-y="true" height="100%">
						<view>
							<view class="tab-item truncate" :class="{ 'tab-item-active': index == tabActive }"
				...
				<!--  #endif -->
				<!--  #ifndef  H5 -->
				<view class="tabs-box z-2 fixed left-0 bg-[#fff] pb-[env(safe-area-inset-bottom)] bottom-[100rpx] top-0"
					:class="{ 'top-[106rpx]': config.search.control, '!bottom-[198rpx]': config.cart.control && config.cart.event === 'cart' }">
					<scroll-view :scroll-y="true" height="100%">
						<view>
							<view class="tab-item" :class="{ 'tab-item-active': index == tabActive }"
								v-for="(item, index) in tabsData" :key="item.site_id" @click="firstLevelClick(index, item)">
								<view class="text-box">
									{{ item.category_name }}
								</view>
				...
				<!--  #endif -->
				<view class="flex justify-center flex-wrap pl-[182rpx]"
					...
					<template v-for="(item, index) in list" :key="item.goods_id">
						<view
							...
							<u--image class="overflow-hidden  rounded-md" width="168rpx" height="168rpx"
								:src="img(item.goods_cover_thumb_small ? item.goods_cover_thumb_small : '')"
								model="aspectFill">
								<template #error>
					...
					<mescroll-empty :option="{ 'icon': img('static/resource/images/empty.png') }"
						v-if="!list.length && !loading && listLoading"></mescroll-empty>
			...
			<view v-if="config.cart.control && config.cart.event === 'cart'"
				...
				<button
					class="w-[180rpx] h-[70rpx] text-[30rpx] leading-[70rpx] text-[#fff] m-0 border-0 bg-color remove-border"
...
import { getGoodsCategoryTree, getGoodsPages } from '@/shop/api/goods';
...
const getListFn = (mescroll: mescrollStructure) => {
	...
	getGoodsPages({
		...
		limit: mescroll.size,
		keyword: '', // 搜索关键词
		goods_category: category_id.value, // 商品分类id
		brand_id: '', // 品牌id
		label_ids: '', // 标签id
		start_price: '', // 价格开始区间
		end_price: '', // 价格结束区间
...
const toLink = (goods_id: string) => {
	redirect({ url: '/shop/pages/goods/detail', param: { goods_id } })
}
...
const searchNameFn = () => {
	// getMescroll().resetUpScroll();
	redirect({ url: '/shop/pages/goods/list', param: { goods_name: searchName.value } })
}
//点击商品购物车按钮
let cartRef = ref()
const itemCart = (row: any, id: any) => {
	...
	if (!userInfo.value) {
		useLogin().setLoginBack({ url: '/shop/pages/goods/category' })
		return false
	}
...
//进入购物车
const toCart = () => {
	redirect({ url: '/shop/pages/goods/cart' })
}
