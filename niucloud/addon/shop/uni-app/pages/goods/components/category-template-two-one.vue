	...
	<view class="bg-[#f8f8f8] min-h-screen">
		<view class="mescroll-box" v-if="tabsData.length">
			...
			<scroll-view :scroll-y="true">
				<view class="flex flex-wrap pl-[182rpx] pb-[70rpx] pt-[20rpx]"
					...
					<template v-for="(item, index) in tabsData[tabActive]?.child_list" :key="item.category_id">
						<view class="w-[160rpx] ml-[22rpx] mb-[24rpx]" @click="toLink(item.category_id)">
							<u--image width="160rpx" height="160rpx" :src="img(item.image ? item.image : '')"
								model="aspectFill">
								<template #error>
...
import { getGoodsCategoryTree } from '@/shop/api/goods';
...
const toLink = (curr_goods_category: string) => {
	redirect({ url: '/shop/pages/goods/list', param: { curr_goods_category } })
}
// 搜索名字
const searchNameFn = () => {
