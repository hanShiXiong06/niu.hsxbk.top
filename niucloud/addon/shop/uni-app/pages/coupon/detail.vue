<template>
    ...
    class="min-h-screen bg-style relative"
        ...
        <view class="absolute top-[34%] w-[100%]">
            ...
            <view class="text-[52rpx] text-color mt-[32rpx] text-center price-font">￥{{ coupon_price || 0.00 }}</view>
            ...
            <view class="flex justify-center items-center mt-[80rpx]">
                <u-button v-if="detail.btnType === 'collected'"
                    class="!w-[500rpx] !h-[80rpx] !text-[28rpx] !m-0 !border-0 !mr-[34rpx] !bg-[#93979D] !border-[#93979D]"
                    type="primary" shape="circle" disabled>该优惠券领取已达到上限</u-button>
                <u-button v-if="detail.btnType === 'collecting'"
                    class="!w-[500rpx] !h-[80rpx] !text-[28rpx] !m-0 !border-0 !mr-[34rpx] !bg-[#FF4646]" type="primary"
                    shape="circle" @click="collecting(detail.id)">领取</u-button>
                <u-button v-if="detail.btnType === 'using'"
                    class="!w-[500rpx] !h-[80rpx] !text-[28rpx] !m-0 !border-0 !mr-[34rpx] !bg-[#FF4646]" type="primary"
                    shape="circle" @click="toLink(detail.id)">去使用</u-button>
            </view>
...
import { getShopCouponInfo, getCoupon } from '@/shop/api/coupon'
...
onLoad((option) => {
    if(!option.coupon_id){
        ...
        setTimeout(() => {
				redirect({ url: '/shop/pages/index', mode: 'reLaunch' })
			}, 600)
        return false
    }else{
...
const collecting = (coupon_id: any) => {
    if (!userInfo.value) {
        useLogin().setLoginBack({ url: '/shop/pages/coupon/list' })
        return false
    }
...
const toLink = (id: any) => {
    redirect({ url: '/shop/pages/goods/list', param: { coupon_id: id } })
}
</script>
