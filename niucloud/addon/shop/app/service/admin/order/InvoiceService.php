...
class InvoiceService extends BaseAdminService
{
    ...
    {
        $order = 'id desc';
        $search_model = $this->model->withSearch([ "is_invoice", "header_type", "header_name", "create_time", "invoice_time" ], $where)->where([ [ 'status', '=', 2 ] ])->append([ 'header_type_name', 'type_name' ])->field('*')->order($order);
        return $this->pageQuery($search_model);
    }

    public function getInfo(int $id)
    {
        $detail = $this->model->where([ [ 'id', '=', $id ], [ 'status', '=', 2 ] ])->field('*')->append([ 'header_type_name', 'type_name' ])->findOrEmpty()->toArray();
        return $detail;
    }
