...
//                $update_data['timeout'] = 0;
    ...
    public function auditRefundGoods($data)
    {
        ...
        if ($order_refund_info->isEmpty()) throw new CommonException('SHOP_ORDER_REFUND_IS_INVALID');//退款已失效
        if ($order_refund_info[ 'status' ] != OrderRefundDict::STORE_REFUSE_REFUND_GOODS_APPLY_WAIT_BUYER) throw new CommonException('SHOP_ORDER_REFUND_IS_ONLY_WAIT_REFUND_GOODS');//退款已失效(只有待确认收货请求才可以修改退款)
