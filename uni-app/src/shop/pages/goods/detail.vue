<template>
	<view class="bg-[#f6f6f6] min-h-[100vh] relative" v-if="Object.keys(goodsDetail).length">
		<u-swiper :list="goodsDetail.goods.goods_image" :autoplay="false" height="600rpx"></u-swiper>
		...
		<view
			...
			<view class="flex items-center">
				<view class="flex flex-col justify-center items-center mr-[39rpx]"
					@click="redirect({ url: '/shop/pages/index' })">
				...
				<view class="flex flex-col justify-center items-center"
					@click="redirect({ url: '/shop/pages/goods/cart' })">
					<view class="relative">
...
import { getGoodsDetail, collect, cancelCollect, getEvaluateList } from '@/shop/api/goods';
import { getShopCouponList, getCoupon } from '@/shop/api/coupon';
import nsGoodsSku from '@/shop/components/ns-goods-sku/ns-goods-sku.vue';
...
const collectFn = () => {
	// 检测是否登录
	if (!userInfo.value) {
		useLogin().setLoginBack({ url: '/shop/pages/goods/detail', param: { sku_id: goodsDetail.value.sku_id } })
		return false
	}
...
//进入评论
const toLink = () => {
	redirect({ url: '/shop/pages/evaluate/list', param: { goods_id: goodsDetail.value.goods_id } })
}
