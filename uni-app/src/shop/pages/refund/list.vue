						...
						<view class="order-goods-item flex mt-[30rpx] px-[24rpx]">
							<view class="w-[160rpx] h-[160rpx] flex-2">
								<u--image class="rounded-[10rpx] overflow-hidden" width="160rpx" height="160rpx"
									:src="img(item.order_goods.sku_image?item.order_goods.sku_image:'')" model="aspectFill">
									<template #error>
...
<script setup lang="ts">
	...
	import { getRefundList, closeRefund } from '@/shop/api/refund';
	...
	const toLink = (data) => {
		redirect({ url: '/shop/pages/refund/detail', param: { order_refund_no : data.order_refund_no } })
	}
	...
	const refundBtnFn = (data, type) => {
		if(type == 'cancel'){
			...
			redirect({ url: '/shop/pages/refund/edit_apply', param: { order_refund_no : data.order_refund_no } })
		}else if(type == 'logistics'){
			redirect({ url: '/shop/pages/refund/detail', param: { order_refund_no : data.order_refund_no, type: 'logistics' } })
		}else if(type == 'editLogistics'){
			redirect({ url: '/shop/pages/refund/detail', param: { order_refund_no : data.order_refund_no, type: 'logistics', is_edit_delivery: true } })
		}
	}
