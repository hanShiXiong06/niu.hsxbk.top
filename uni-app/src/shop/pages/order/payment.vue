<template>
    <scroll-view scroll-y="true" class="bg-page h-screen" v-if="orderData">
        <view class="py-[20rpx] px-[24rpx]" style="background: linear-gradient(var(--primary-color) 0%, var(--primary-color-disabled) 7%, #F4F4F6 10%)">
            <!-- 配送方式 -->
            <view class="mb-[20rpx] rounded-[16rpx] bg-white" v-if="orderData.basic.has_goods_types.includes('real')">
                <view class="flex items-center rounded-tl-[16rpx] rounded-tr-[16rpx] overflow-hidden w-full">
                    <view v-for="(item, index) in delivery_type_list" class="flex-1"
                        ...
                        <!-- <u-button :text="item.name" type="primary" v-if="createData.delivery.delivery_type == item.key" @click="switchDeliveryType(item.key)"></u-button>
                    <u-button :text="item.name" v-else></u-button> -->
                    </view>
                </view>
                <view class="p-[24rpx]">
                    <!-- 收货地址 -->
            ...
            <view class="mb-[20rpx] p-[24rpx] rounded-md bg-white flex">
                ...
                <view class="flex-1">
                    <input type="text" v-model="createData.member_remark" class="text-right text-[28rpx]"
...
import { orderCreateCalculate, orderCreate } from '@/shop/api/order'
    ...
    orderCreate(createData.value)
        .then(({ data }) => {
            ...
            if (orderData.value.basic.order_money == 0) {
                redirect({ url: '/shop/pages/order/detail', param: { order_id: orderId }, mode: 'redirectTo' })
            } else {
                payRef.value?.open(data.trade_type, data.order_id, `/shop/pages/order/detail?order_id=${data.order_id}`)
            }
        })
        .catch(() => {
