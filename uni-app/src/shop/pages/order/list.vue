...
import { getShopOrderStatus, getShopOrder, orderClose, orderFinish } from '@/shop/api/order';
import { getEvaluateConfig } from '@/shop/api/shop';
...
const toLink = (data) => {
	redirect({ url: '/shop/pages/order/detail', param: { order_id: data.order_id } })
}
...
const orderBtnFn = (data, type = '') => {
	if (type == 'pay')
		payRef.value?.open(data.order_type, data.order_id);
	else if (type == 'close') {
		close(data);
	} else if (type == 'finish') {
		finish(data);
		...
		if (!data.is_evaluate) {
			redirect({ url: '/shop/pages/evaluate/order_evaluate', param: { order_id: data.order_id } })
		} else {
			redirect({ url: '/shop/pages/evaluate/order_evaluate_view', param: { order_id: data.order_id } })
		}
