		...
		<el-card class="box-card !border-none" shadow="never">
			...
			<el-tabs v-model="activeName" class="" @tab-click="tabHandleClick">
				<el-tab-pane :label="t('basicInfoTab')" name="basic">
					...
					<el-form :model="formData" label-width="120px" ref="basicFormRef" :rules="formRules"
					         class="page-form">
						<el-form-item :label="t('goodsType')" v-if="formData.goods_id">
							<template v-for="(item,index) in goodsType" :key="item.type">
						...
						<el-form-item :label="t('goodsType')" v-else>
							<div class="goods-type-wrap" :class="{ 'selected' : formData.goods_type == item.type }"
							     v-for="(item,index) in goodsType" :key="item.type"
						...
						<el-form-item :label="t('goodsCategory')" prop="goods_category">
							...
							<div class="ml-[10px]">
								...
								<span class="cursor-pointer text-primary"
								      @click="toGoodsCategoryEvent">{{ t('addGoodsCategory') }}</span>
							</div>
						</el-form-item>

						<el-form-item :label="t('brand')">
							<el-select v-model="formData.brand_id" :placeholder="t('brandPlaceholder')"
							           clearable>
								<el-option
							...
							<div class="ml-[10px]">
								...
								<span class="cursor-pointer text-primary"
								      @click="toGoodsBrandEvent">{{ t('addGoodsBrand') }}</span>
							</div>
						</el-form-item>
						<el-form-item :label="t('label')">
							<el-checkbox-group v-model="formData.label_ids">
								<el-checkbox :label="item.label_id" v-for="item in labelOptions">{{item.label_name }}
								</el-checkbox>
							</el-checkbox-group>
							<div class="ml-[10px]">
								<span class="cursor-pointer text-primary mr-[10px]" @click="refreshGoodsLabel">{{ t('refresh') }}</span>
								<span class="cursor-pointer text-primary"
								      @click="toGoodsLabelEvent">{{ t('addGoodsLabel') }}</span>
							</div>
						</el-form-item>
						<el-form-item :label="t('goodsService')">
							<el-checkbox-group v-model="formData.service_ids">
								<el-checkbox :label="item.service_id" v-for="item in serviceOptions">{{item.service_name
									}}
								</el-checkbox>
							</el-checkbox-group>
							<div class="ml-[10px]">
								...
								<span class="cursor-pointer text-primary"
								      @click="toGoodsServiceEvent">{{ t('addGoodsService') }}</span>
							</div>
						</el-form-item>

						<el-form-item :label="t('supplier')"
						              v-if="formData.addon_shop_supplier && formData.addon_shop_supplier.status == 1">
							<el-select v-model="formData.supplier_id" :placeholder="t('supplierPlaceholder')"
							           clearable>
								<el-option
							...
							<div class="ml-[10px]">
								...
								<span class="cursor-pointer text-primary"
								      @click="toSupplierEvent">{{ t('addSupplier') }}</span>
							</div>
						</el-form-item>
						...
						<el-form-item :label="t('unit')" prop="unit">
							<el-input v-model="formData.unit" clearable :placeholder="t('unitPlaceholder')"
							          class="input-width" show-word-limit
							          maxlength="6"
							/>
						</el-form-item>
					...
					<el-form :model="formData" label-width="120px" ref="priceStockFormRef" :rules="formRules"
						...
						<template v-if="formData.spec_type == 'single'">
							...
							<el-form-item :label="t('marketPrice')" prop="market_price">
								<el-input v-model="formData.market_price" clearable placeholder="0.00"
							...
							<el-form-item :label="t('costPrice')" prop="cost_price">
								<el-input v-model="formData.cost_price" clearable placeholder="0.00"
								          class="input-width" type="number"
								          maxlength="10">
									<template #append>{{ t('yuan') }}</template>
								</el-input>
							</el-form-item>
							<el-form-item :label="t('weight')" prop="weight">
								<el-input v-model="formData.weight" clearable placeholder="0.000"
								          class="input-width" type="number"
								          maxlength="20">
							...
							<el-form-item :label="t('volume')" prop="volume">
								<el-input v-model="formData.volume" clearable placeholder="0.000"
								          class="input-width" type="number"
								          maxlength="20">
							...
							<div class="spec-wrap">
											...
											<ul ref="specValueRef">
												<li :class="'draggable-element'+index"
													...
													<el-input v-model="specValue.spec_value_name" clearable
													          :placeholder="t('specValueNamePlaceholder')"
													          class="input-width"
													          :suffix-icon="Rank"
													          maxlength="20"
								...
								<div class="batch-operation-sku" v-show="Object.keys(goodsSkuData).length">
									...
									<el-select v-model="batchOperation.spec" class="set-spec-select">
										...
										<template v-for="(item,key) in goodsSkuData"
										          :key="key">
											<el-option
													v-if="item.spec_name"
									...
									<el-input v-model="batchOperation.price" clearable
									          :placeholder="t('price')" class="set-input" type="number"
									          maxlength="10"
									/>
									<el-input v-model="batchOperation.market_price" clearable
									          :placeholder="t('marketPrice')" class="set-input" type="number"
									          maxlength="10"
									/>
									<el-input v-model="batchOperation.cost_price" clearable
									          :placeholder="t('costPrice')" class="set-input" type="number"
									          maxlength="10"
									/>
									<el-input v-model="batchOperation.stock" clearable
									          :placeholder="t('stock')" class="set-input" type="number"
									          maxlength="10"
									/>
									<el-input v-model="batchOperation.weight" clearable
									          :placeholder="t('skuWeight')" class="set-input" type="number"
									          maxlength="20"
									/>
									<el-input v-model="batchOperation.volume" clearable
									          :placeholder="t('skuVolume')" class="set-input" type="number"
									          maxlength="20"
									/>
									<el-input v-model="batchOperation.sku_no" clearable
									          maxlength="50"
											...
											<div class="el-table__header-wrapper">
												<table class="el-table__header" border="0" cellpadding="0"
													...
													<tr>
														...
														<template v-for="(item,index) in goodsSpecFormat">
														...
														<div class="el-scrollbar__view"
															...
															<table class="el-table__body" cellspacing="0"
																...
																<tr class="el-table__row"
																    ...
																    :key="key">
																	<template v-for="(c,cIndex) in specData">
					...
					<el-form :model="formData" label-width="120px" ref="deliveryFormRef" :rules="formRules"
						...
						<el-form-item :label="t('deliveryType')" prop="delivery_type">
							<el-checkbox-group v-model="formData.delivery_type">
								<el-checkbox v-for="item in deliveryTypeCheckBox" :label="item.key">{{ item.name }}
								</el-checkbox>
							</el-checkbox-group>
						</el-form-item>
						...
						<el-form-item :label="t('isFreeShipping')"
							...
							<el-switch v-model="formData.is_free_shipping" :active-value="1"
							           :inactive-value="0"/>
						</el-form-item>
						...
						<el-form-item :label="t('deliveryTemplateId')" prop="delivery_template_id"
							...
							<div class="ml-[10px]">
								...
								<span class="cursor-pointer text-primary"
								      @click="toDeliveryTemplateEvent">{{ t('addDeliveryTemplateId') }}</span>
							</div>
						</el-form-item>
					</el-form>
				...
				<el-tab-pane :label="t('goodsDesc')" name="detail">
					<el-form :model="formData" label-width="120px" ref="detailFormRef" :rules="formRules"
					         class="page-form">
...
<script lang="ts" setup>
        ...
        if (data) {
            ...
            if (formData.goods_id && data.goods_info) {
                ...
                if (formData.spec_type == 'single') {
                    // 单规格
                    let skuInfo = data.goods_info.sku_list[0];
                    ...
                    // 多规格
                    let specList = data.goods_info.spec_list;
                    ...
                    let skuList = data.goods_info.sku_list;
    ...
    const bindSpecValue = () => {
        ...
        for (let i = 0; i < specValueRef.value.length; i++) {
            let item = specValueRef.value[i];
            const sortable = Sortable.create(item, {
                group: 'draggable-element-' + i,
                animation: 200,
                // 结束拖拽
                onEnd: event => {
                    let temp = goodsSpecFormat[i].values[event.oldIndex!];
    ...
    const refreshGoodsSkuData = () => {
        ...
        var tempGoodsSkuData = cloneDeep(goodsSkuData);// 记录原始数据，后续用作对比
        let skuData: any = {};
        for (let spec of arr) {
            var item_prop_arr: any = {};
            if (Object.keys(skuData).length > 0) {
                for (var ele_2 in skuData) {
                    for (var ele_3 of spec['values']) {
                        var sku_spec = cloneDeep(skuData[ele_2].sku_spec);// 防止对象引用
                        sku_spec.push(ele_3);
        ...
        // 比对已存在的规格项/值，并且赋值
        for (let tempKey in tempGoodsSkuData) {
            for (let key in skuData) {
                var count = matchSkuSpecCount(tempGoodsSkuData[tempKey].sku_spec, skuData[key].sku_spec);
                if (count === skuData[key].sku_spec.length) {
                    // 匹配成功后，要同步最新的规格项名称、规格值集合
                    var spec_name = skuData[key].spec_name;
                    var sku_spec = skuData[key].sku_spec;
                    Object.assign(skuData[key], tempGoodsSkuData[tempKey]);
                    skuData[key].spec_name = spec_name;
                    skuData[key].sku_spec = sku_spec;
                    break;
                }
            }
        }

        for (let item in goodsSkuData) {
    ...
    const refreshSkuTable = () => {
        ...
        for (var i = length - 1; i >= 0; i--) {
            for (var k = 0; k < Object.keys(goodsSkuData).length;) {
        ...
        let skuVerify: any = {
            ...
            sku_weight: [{
                ...
                validator: (rule: any, value: any, callback: any) => {
                    if (formData.spec_type == 'multi') {
                        for (let key in goodsSkuData) {
                            if (goodsSkuData[key].weight.length > 0 && (isNaN(goodsSkuData[key].weight) || !regExp.special.test(goodsSkuData[key].weight))) {
                                callback(new Error(t('weightTips')))
                                break;
                            } else if (goodsSkuData[key].weight < 0) {
                                callback(new Error(t('weightNotZeroTips')))
                                break;
                            }
                        }
            ...
            sku_volume: [{
                ...
                validator: (rule: any, value: any, callback: any) => {
                    if (formData.spec_type == 'multi') {
                        for (let key in goodsSkuData) {
                            if (goodsSkuData[key].volume.length > 0 && (isNaN(goodsSkuData[key].volume) || !regExp.special.test(goodsSkuData[key].volume))) {
                                callback(new Error(t('volumeTips')))
                                break;
                            } else if (goodsSkuData[key].volume < 0) {
                                callback(new Error(t('volumeNotZeroTips')))
                                break;
                            }
                        }
    ...
    const save = () => {
        verify(() => {
            ...
            let api = formData.goods_id ? editGoods : addGoods;
            let data = cloneDeep(formData);
            let goods_category: any = [];
            data.goods_category.forEach((item: any) => {
                if (typeof item == 'object') {
                    item.forEach((second: any) => {
                        if (goods_category.indexOf(second) == -1) {
                            goods_category.push(second);
                        }
                    })
                } else {
                    if (goods_category.indexOf(item) == -1) {
                        goods_category.push(item);
                    }
                }
            });
