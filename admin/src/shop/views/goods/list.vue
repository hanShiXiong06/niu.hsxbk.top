					...
					<el-form-item :label="t('skuPrice')">
						<div class="region-input">
							<input type="text" :placeholder="t('startPricePlaceholder')" maxlength="10"
							       v-model="goodsTable.searchParam.start_price">
							<span class="separator">-</span>
							<input type="text" :placeholder="t('endPricePlaceholder')" maxlength="10"
							       v-model="goodsTable.searchParam.end_price">
						</div>
					</el-form-item>
			...
			<div class="mt-[10px]">
				...
				<el-table :data="goodsTable.data" size="large" v-loading="goodsTable.loading"
						...
						<template #default="{ row }">
							<div class="flex items-center cursor-pointer" @click="previewEvent(row)">
								<div class="min-w-[70px] h-[70px] flex items-center justify-center">
									...
									<img v-else class="w-[70px] h-[70px]" src="@/shop/assets/goods_default.png"
									     fit="contain"/>
								</div>
								<div class="ml-2">
					...
					<el-table-column prop="sort" :label="t('sort')" min-width="120" sortable="custom">
						<template #default="{ row }">
							<el-input v-model="row.sort" class="input-width-sort" maxlength="10"
					...
					<el-table-column :label="t('operation')" fixed="right" align="right" min-width="120">
						<template #default="{ row }">
							...
							<el-button type="primary" link @click="deleteEvent(row.goods_id)">{{ t('delete') }}
							</el-button>
						</template>
					</el-table-column>
...
<script lang="ts" setup>
    ...
    import goodsStockEditPopup from '@/shop/views/goods/components/goods-stock-edit-popup.vue'
    import goodsPriceEditPopup from '@/shop/views/goods/components/goods-price-edit-popup.vue'
    import goodsSpreadPopup from '@/shop/views/goods/components/goods-spread-popup.vue'
    ...
    const previewEvent = (data: any) => {
        let url = router.resolve({
            path: '/preview/wap',
            query: {
                page: `/shop/pages/goods/detail?goods_id=${data.goods_id}`
            }
        });
    ...
    const loadGoodsList = (page: number = 1) => {
        ...
        let searchData = cloneDeep(goodsTable.searchParam);
