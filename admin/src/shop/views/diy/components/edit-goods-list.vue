	...
	<div class="content-wrap" v-show="diyStore.editTab == 'content'">
		<div class="edit-attr-item-wrap">
			...
			<div class="flex items-center mb-[18px] rounded overflow-hidden">
				<span class="iconfont icongudingzhanshi border-[1px] border-solid border-[#eee] cursor-pointer flex-1 flex items-center justify-center py-[5px]"
					:class="{'border-[var(--el-color-primary)] text-[var(--el-color-primary)]':diyStore.editComponent.style == 'style1'}"
					@click="diyStore.editComponent.style = 'style1'"
					></span>
				<span class="iconfont icontuwendaohang3 border-[1px] border-solid border-[#eee] cursor-pointer flex-1 flex items-center justify-center py-[5px]"
					:class="{'border-[var(--el-color-primary)] text-[var(--el-color-primary)]':diyStore.editComponent.style == 'style2'}"
					@click="diyStore.editComponent.style = 'style2'"
					></span>
			</div>
		</div>
		<div class="edit-attr-item-wrap">
			...
			<el-dialog v-model="categoryShowDialog" :title="t('goodsCategoryTitle')" width="1000px"
				...
				<el-table :data="categoryTable.data" ref="categoryTableRef" size="large" v-loading="categoryTable.loading" height="490px"
                                    ...
                                    <template #error>
                                        <div class="image-slot">
                                            <img class="w-[30px] h-[30px]"
                                                src="@/shop/assets/category_default.png" />
                                        </div>
                                    </template>
                                </el-image>
                            </div>
...
<script lang="ts" setup>
	import { getCategoryTree } from '@/shop/api/goods'
    import {t} from '@/lang'
	import { img } from '@/utils/common'
    import useDiyStore from '@/stores/modules/diy'
	import {ref, reactive, onMounted} from 'vue'
	import { ElTable } from 'element-plus'
	import goodsSelectPopup from '@/shop/views/goods/components/goods-select-popup.vue'
	
    const diyStore = useDiyStore()
	diyStore.editComponent.ignore = []; // 忽略公共属性

	let categoryShowDialog = ref(false)
	...
	const loadCategoryList = () => {
		...
		getCategoryTree({
			...
			if(diyStore.editComponent.source == 'category'){
				categoryTable.data.forEach((item,index)=>{
