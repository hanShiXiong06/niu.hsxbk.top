    ...
    <el-dialog v-model="showDialog" :title="t('packageInfo')" width="700px"
        ...
        <div v-loading="loading" class="max-h-[600px] overflow-y-auto">
						...
						<template #default="{ row }">
							<div class="flex">
								<div class="flex items-center">
									<img class="w-[50px] h-[50px] mr-[10px]" :src="img(row.goods_image_thumb_small)" />
								</div>
			...
			<div v-if="packageData.traces">
				<div v-if="packageData.traces.list" class="flex justify-between mb-[15px]" v-for="(item, index) in packageData.traces.list" :key="index">
...
import { deliveryPackage } from '@/shop/api/order'
