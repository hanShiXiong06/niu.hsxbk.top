    ...
    <div class="main-container">
        <el-card class="box-card !border-none" shadow="never">
            ...
            <el-card class="box-card !border-none mb-[10px] table-search-wrap" shadow="never">
                <el-form :inline="true" :model="orderTableData.searchParam" ref="searchFormRef">
                    <el-form-item :label="t('cashOutNumber')" prop="cash_out_no">
					    <el-input v-model="orderTableData.searchParam.cash_out_no" class="w-[240px]" :placeholder="t('cashOutNumberPlaceholder')" />
					</el-form-item>
            ...
            <div class="mt-[10px]">
                <el-table :data="orderTableData.data" size="large" v-loading="orderTableData.loading">
                    ...
                    <el-table-column :label="t('cashOutMethod')"  min-width="160" >
                        <template #default="{ row }">
                            <div class="flex items-center">
                                {{ row.transfer_bank || Transfertype[row.transfer_type].name }}
                            </div>
                            <div class="flex items-center">
                    ...
                    <el-table-column :label="t('cashOutMethodType')"  min-width="160" >
                    <template #default="{ row }">
                            <div class="flex items-center">
                    ...
                    <el-table-column :label="t('money')" align="center" min-width="200" >
                        <template #default="{ row }">
                            <div class="flex flex flex-col" align="left">
                                <span>申请提现金额：{{ row.apply_money || '0.00' }}</span>
                                <span>实际转账金额：{{ Number.parseFloat(row.money).toFixed(2)  || '0.00' }}</span>
                                <span>提现手续费：{{ Number.parseFloat(row.service_money).toFixed(2)  || '0.00' }}</span>
                            </div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column :label="t('operation')" fixed="right" width="200" align="right">
                        <template #default="{ row }">
        ...
        <el-dialog v-model="cashOutShowDialog" :title="t('cashOutDetail')" width="500px" :destroy-on-close="true">
            <el-form :model="cashOutInfo" label-width="120px" ref="formRef" :rules="formRules" class="page-form" v-loading="cashOutLoading">
                ...
                <el-form-item :label="t('cashOutMethodType')">
                    <div class=" input-width"> {{ cashOutInfo.transfer_type_name  }} </div>
                </el-form-item>
                <el-form-item :label="t('cashOutMethod')">
                    <div class="input-width"> {{ cashOutInfo.transfer_bank || cashOutInfo.transfer_type_name }} </div>
                </el-form-item>
                <el-form-item :label="t('transferAccount')">
                ...
                <el-form-item :label="t('reasonsRefusal')" v-if="cashOutInfo.status == -1">
                    <div class="input-width"> {{ cashOutInfo.refuse_reason }} </div>
                </el-form-item>
                <el-form-item :label="t('applyTime')" v-if="cashOutInfo.create_time">
                    <div class="input-width"> {{ cashOutInfo.create_time || ''}} </div>
                </el-form-item>
                <el-form-item :label="t('auditTime')" v-if="cashOutInfo.audit_time">
                    <div class="input-width"> {{ cashOutInfo.audit_time || ''}} </div>
                </el-form-item>
                <el-form-item :label="t('transferTime')" v-if="cashOutInfo.transfer_time">
                    <div class="input-width"> {{ cashOutInfo.transfer_time || ''}} </div>
                </el-form-item>
                <el-form-item :label="t('transferVoucher')" v-if="cashOutInfo.status == 3">
					<el-image v-if="cashOutInfo.transfer.transfer_voucher" style="width: 150px; height: 150px" :src="img(cashOutInfo.transfer.transfer_voucher)" fit="contain" :preview-src-list="[img(cashOutInfo.transfer.transfer_voucher)]"></el-image>
                    <span v-else>无</span>
                </el-form-item>
            </el-form>
            <template #footer>
                <span class="dialog-footer">
                    <el-button type="primary" @click="cashOutShowDialog = false">{{t('close')}}</el-button>
                </span>
            </template>
        </el-dialog>
